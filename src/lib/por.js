...
function _extractTxParams(tx) {
  ...
  const sequence = tx.inputs[tx.inputs.length-1].sequence.toString(16).padStart(8, '0').reverseHex()
  const nOuts = tx.outputs.length.toString(16).padStart(2,'0')  // assume that nOuts fits in 1 byte
  const value = tx.outputs[0].value.toString(16).padStart(16, '0').reverseHex()
